...
async def upload_file(client, message, file_path, user_mention, task_info=None, batch_info=None, overall_current=0, overall_total=0, start_time=None):
    try:
        ...
        if active_dump:
            ...
            try:
                # ✅ FIX: 'thumbnail' ki jagah wapas 'thumb' kiya Heroku compatibility ke liye
                if file_name.lower().endswith(('.mp4', '.mkv', '.webm')):
        ...
        if mode == "leech" or (url and ("magnet:" in url or ".torrent" in url.lower())):
            ...
            while True:
                ...
                if status.status == "complete":
                    # ✅ FIX: Errno 21 aur 700 JPGs wala filter (Sirf selected paths pass honge)
                    selected_paths = []
...
async def process_task(client, message, url, mode="auto", upload_target="tg", task_info=None, format_id=None, status_msg=None):
    ...
    try:
        ...
        # ✅ NAYA FIX: FOLDER EXTRACTION & SELECTOR LOGIC (Kachra JPGs hatane ke liye)
        final_files = []
        if isinstance(file_path, list):
            final_files = file_path # Seedha selected files ki list utha li
        ...
        # ✅ FIX: [Errno 21] Is a directory ka permanent ilaaj
        if isinstance(file_path, list):
            try:
...
async def ytdl_selector(c, m):
    ...
    try:
        ...
        # ✅ NAYA FIX: YTDL Block Error Catcher
