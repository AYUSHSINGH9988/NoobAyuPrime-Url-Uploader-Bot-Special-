...
async def convert_to_streamable(file_path, message):
    """Convert wmv/m4v/avi/f4v/wvm → mp4 using ffmpeg. Stream copy first, re-encode as fallback."""
    ...
    try:
        ...
        if proc.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            os.remove(file_path)
            return output_path, True
        # Attempt 2: re-encode (slower but more compatible)
...
async def upload_file(client, message, file_path, user_mention, task_info=None, batch_info=None,
    ...
    try:
        ...
        # Convert wmv/m4v/avi/f4v/wvm → mp4 (video ke roop mein upload ke liye)
