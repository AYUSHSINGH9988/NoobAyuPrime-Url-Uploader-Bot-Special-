...
async def scriptdl_handler(c, m):
    ...
    if "hentaicity.com" in url:
        ...
        return await msg.edit_text(
            ...
            "Currently supports: HentaiCity, PornHub, WatchHentai & Dailymotion.\n\n"
    ...
    async def _run_scriptdl():
        try:
            if is_batch:
                ...
                # Final summary in the same persistent status message.
                await _safe_status_edit(
                    status_msg,
...
async def tgdl_cmd(c, m):
    ...
    status = await m.reply_text(
        f"📥 <b>TGDL</b>\nChat: <code>{chat_a}</code>\nRange: <code>{mid_a}-{mid_b}</code>\n\nStarting..."
    )
        ...
        for mid in range(mid_a, mid_b + 1):
            try:
                ...
                if gid:
                    ...
                    seen_groups.add(gid)
                    try:
                        group = await user_app.get_media_group(chat_a, mid)
                    except Exception:
                        group = [src]
                    files = []
                    for g in group:
                        try:
                            fp = await user_app.download_media(g, in_memory=False)
                            if fp:
                                files.append((g, fp))
                    ...
                    try:
                        await c.send_media_group(m.chat.id, media)
                        ok += 1
                    except Exception as _se:
                        fail += 1
                    ...
                    if src.text:
                        try:
                            await c.send_message(m.chat.id, src.text.html or src.text)
                            ok += 1
                        except Exception:
                            fail += 1
                        continue
                    fp = None
                    try:
                        fp = await user_app.download_media(src, in_memory=False)
                        if not fp:
                            fail += 1
                            continue
                        cap = src.caption or ""
                        if src.photo:
                            await c.send_photo(m.chat.id, fp, caption=cap)
                        elif src.video:
                            await c.send_video(m.chat.id, fp, caption=cap)
                        elif src.audio:
                            await c.send_audio(m.chat.id, fp, caption=cap)
                        elif src.voice:
                            await c.send_voice(m.chat.id, fp, caption=cap)
                        elif src.animation:
                            await c.send_animation(m.chat.id, fp, caption=cap)
                        else:
                            await c.send_document(m.chat.id, fp, caption=cap)
                        ok += 1
                    except Exception as _se:
                        fail += 1
                ...
                fail += 1
                print(f"[tgdl] loop err: {_le}")
            with suppress(Exception):
                await status.edit_text(
                    f"📥 <b>TGDL</b>\nProgress: <code>{mid - mid_a + 1}/{mid_b - mid_a + 1}</code>\n"
                    f"✅ {ok} • ❌ {fail}"
                )
    finally:
        if started:
            with suppress(Exception): await user_app.stop()
    await status.edit_text(f"🏁 <b>TGDL done</b>\n✅ {ok} • ❌ {fail}")
