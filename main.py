    ...
    while task_queue:
        ...
        try:
            ...
            if is_reply_file:
                ...
                await msg.edit_text(f"📥 <b>Downloading from Telegram...</b> ⚡\n<code>{clean_html(original_filename)}</code>")
                ...
                if "magnet:" in link or link.endswith(".torrent"):
                    if not aria2: 
                        await msg.edit_text("❌ <b>Aria2 is not running!</b>")
                        continue
                    download = aria2.add_magnet(link)
                    while not download.is_complete:
                        download.update()
                        if download.status == "error": 
                            await msg.edit_text("❌ <b>Aria2 Download Error</b>")
                            file_path = None
                            break
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.get(link) as resp:
                            if resp.status != 200: 
                                await msg.edit_text("❌ <b>Invalid Link / Error</b>")
                                continue
            ...
            if not file_path or not os.path.exists(file_path):
                await msg.edit_text("❌ <b>Download Failed or Skipped!</b>")
                continue

            # --- EXTRACTION & PINNING STEP ---
            await msg.edit_text("📦 <b>Extracting & Sorting...</b> 🗂️")
            ...
            if not final_files:
                await msg.edit_text("❌ <b>Empty Archive or No Files Found!</b>")
            ...
            for i, file in enumerate(final_files):
                ...
                thumb = None
                is_video = fname.lower().endswith(('.mp4', '.mkv', '.webm', '.avi', '.mov', '.flv'))
            ...
            await msg.edit_text("✅ <b>Task Completed Successfully!</b> 🎉")
            ...
            traceback.print_exc()
            await msg.edit_text(f"❌ <b>Critical Error:</b> {e}")
            try:
                if 'output_dir' in locals() and os.path.exists(output_dir): shutil.rmtree(output_dir)
            except: pass

    is_processing = False
    await client.send_message(message.chat.id, "🏁 <b>Queue Empty! All tasks finished.</b> 💤")
...
async def add_to_queue(client, message):
    ...
    if message.reply_to_message and message.reply_to_message.document:
        ...
        return await message.reply_text("❌ <b>Send a Link or Reply to a File!</b>")
...
async def start(c, m):
    await m.reply_text(
        "👋 <b>Welcome to Ultra Uploader Bot!</b> 🚀\n\n"
        "Send me any **Link** or **Reply to a Telegram File**.\n"
        "I will:\n"
        "1️⃣ Queue it ⏳\n"
        "2️⃣ Download & Extract (if Zip) 📦\n"
        "3️⃣ Pin the Folder Name in Dump 📌\n"
        "4️⃣ Upload files in Sequence 📡"
    )
