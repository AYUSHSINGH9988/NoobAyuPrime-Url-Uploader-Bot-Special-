...
# Speed boost ke liye workers limit increase ki gayi hai
app = Client("my_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, parse_mode=enums.ParseMode.HTML, workers=100)
...
async def get_bunkr_direct_link(url):
    try:
        slug_match = re.search(r'\/f\/(.*?)$', url)
        if not slug_match:
            slug_match = re.search(r'\/v\/(.*?)$', url)
        if not slug_match: return None
            
        slug = slug_match.group(1).split('?')[0].strip()
        headers = {
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post("https://bunkr.cr/api/vs", json={'slug': slug}, headers=headers) as r:
                if r.status != 200: return None
                encryption_data = await r.json()
    ...
    try:
        ...
        if active_dump:
            ...
            try:
                ...
                try: await message.reply_text(f"❌ <b>Upload Error for {file_name}:</b>\n<code>{e}</code>")
                except: pass
                return False
        else:
...
async def download_logic(url, message, user_id, mode, task_info=None, format_id=None):
    try:
        ...
        if mode == "bunkr" or "bunkr" in url.lower():
            await message.edit_text("🔍 <b>Decrypting Bunkr Link...</b>")
            bunkr_link = await get_bunkr_direct_link(url)
            if not bunkr_link:
                return "ERROR: Failed to extract Bunkr direct link. Encrypted slug might be invalid."
            url = bunkr_link 
            mode = "auto" 
        ...
        if mode == "leech" or (url and ("magnet:" in url or ".torrent" in url.lower())):
                ...
                if url.startswith("http"):
                    async with aiohttp.ClientSession() as session:
                        async with session.get(url, headers=headers) as resp:
                            if resp.status == 200:
                                # FIX FOR BENCODE ERROR: Check if site returned HTML/Login page instead of .torrent
                                content_type = resp.headers.get('Content-Type', '').lower()
                                if 'text/html' in content_type:
                                    return "ERROR: The link returned a Webpage/HTML (Login required or Cloudflare protected), NOT a valid .torrent file."
            ...
            while True:
                ...
                if status.status == "complete":
                    ...
                    if len(selected_paths) > 1: return selected_paths
                    elif len(selected_paths) == 1: return str(selected_paths[0])
                    else: return "ERROR: No downloaded files found"
...
async def process_task(client, message, url, mode="auto", upload_target="tg", task_info=None, format_id=None, status_msg=None):
    ...
    try:
        if upload_target == "tg":
            active_dump = await get_active_dump(message.chat.id)
            if not active_dump:
                await msg.edit_text("❌ <b>No Dump Selected!</b>\nUse /setdump -100xxxxxxxx to add a channel.")
                return
        ...
        traceback.print_exc()
        await msg.edit_text(f"⚠️ <b>Error:</b> <code>{e}</code>")

# ==========================================
#           COMMAND HANDLERS
# ==========================================
# WZML-X Style Direct Dump Setting Fix
@app.on_message(filters.command("setdump"))
async def set_dump_info(c, m):
    if len(m.command) < 2:
        return await m.reply_text("❗ <b>Usage:</b> <code>/setdump -1001234567890</code>\n\nMake sure the bot is an admin in that channel/group first!")
    
    try:
        chat_id = int(m.command[1])
        msg = await m.reply_text("🔍 Checking bot permissions in that channel...")
        
        try:
            # Bot explicitly checks if it has access
            chat = await c.get_chat(chat_id)
            title = chat.title
            await add_dump(m.chat.id, chat_id, title)
            await msg.edit_text(f"✅ <b>Dump Channel Set:</b> {title} ({chat_id})\nNow tasks will directly upload here!")
        except Exception as e:
            await msg.edit_text(f"❌ <b>Error:</b> Cannot access that chat. Ensure bot is Admin there!\n\nReason: <code>{e}</code>")
    except ValueError:
        await m.reply_text("❌ <b>Invalid ID!</b> Must be a number like -100xxxxxxxxxx")
...
# Link Split Fix Applied Here
@app.on_message(filters.command(["leech", "dl", "rclone", "queue", "zip", "compress", "bdl"]))
async def command_handler(c, m):
    ...
    if is_reply:
        ...
        # Split by New Line (\n) to avoid breaking URLs with spaces
...
async def start_cmd(c, m):
    welcome_text = (
        ...
        "• <code>/bdl</code> - For Bunkr / Cyberdrop Links\n"
        "• <code>/ytdl</code> - For YouTube Videos\n"
        "• <code>/queue</code> - Add multiple links (line by line)\n"
        "• <code>/setdump -100xxx</code> - Set upload channel\n"
    )
