...
async def mega_login(email, password):
    ...
    def _megacmd_login():
        ...
        # FIX: Force-logout first to clear any corrupt/stale session
...
def _gdown_blocking(url: str, out_dir: str) -> str | None:
    """Synchronous gdown download. Returns the absolute path of the downloaded
    file on success, or None on failure."""
    try:
        import gdown
    except Exception as _ie:
        raise Exception(
            "gdown not installed. Run: pip install -r requirements.txt"
        ) from _ie
    os.makedirs(out_dir, exist_ok=True)
    cwd = os.getcwd()
    try:
        os.chdir(out_dir)
        # FIX: Handle TypeError for older gdown versions that don't support fuzzy=
        try:
            result = gdown.download(url=url, quiet=True, fuzzy=True)
        except TypeError:
            result = gdown.download(url=url, quiet=True)
        if not result:
            # Try folder mode.
            try:
                listing = gdown.download_folder(url=url, quiet=True, use_cookies=False)
...
async def _gdrive_upload_with_token(local_path: str, token_path: str) -> str:
    """Upload a single local file to the user's Google Drive root using their
    token.pickle. Returns the new file's webViewLink."""
    def _sync():
        import pickle
        from googleapiclient.discovery import build
        from googleapiclient.http import MediaFileUpload
        # FIX: Catch corrupted/text pickle files (\xef BOM / invalid load key error)
        try:
...
async def _safe_status_edit(status_msg, text):
    with suppress(Exception):
...
async def scriptdl_quality_cb(c, cb):
    ...
    async def _process_one_silent(vid_url, fmt_id, status_msg=None, total_v=0,
                                    hook_override=None):
        """Sequential batch download + upload.
        hook_override: optional yt-dlp progress hook (for live progress in status_msg).
        Returns None on success, or an error-reason string on failure."""
        os.makedirs("downloads", exist_ok=True)
        dl_dir = os.path.join("downloads", secrets.token_hex(4))
        os.makedirs(dl_dir, exist_ok=True)
        ...
        try:
            try:
                result = await asyncio.wait_for(
                    asyncio.to_thread(
                        _blocking_download, vid_url, fmt_id, out_tmpl, hook_override, False
                    ),
                    timeout=600,
                )
            except (asyncio.TimeoutError, TimeoutError):
    ...
    async def _run_scriptdl():
        try:
            if is_batch:
                # ─── PHub Batch: WZML-style sequential + standard progress bar + auto-retry ───
                total_v    = len(batch_urls)
                status_msg = msg
                await _safe_status_edit(status_msg,
                    _phub_progress_text(0, 0, total_v, label="PHub Batch"))

                # ── Phase 1: Process all videos sequentially ──
                for i, vid_url in enumerate(batch_urls, start=1):
                    ...
                    try:
                        ...
                        batch_counters["failed"] += 1
                        batch_errors.append(f"Resolve Failed: {vid_url[:80]}")
                        await _safe_status_edit(status_msg,
                    ...
                    # Sequential download+upload with live progress in status_msg
                    loop = asyncio.get_running_loop()
                    start_dl = time.time()
                    def _make_hook(_smsg, _start):
                        def _hook(d):
                            if d["status"] != "downloading": return
                            total_b = d.get("total_bytes") or d.get("total_bytes_estimate") or 0
                            current = d.get("downloaded_bytes") or 0
                            fname   = os.path.basename(d.get("filename") or "Video")
                            if current > 0:
                                b_info = (f"✅ {batch_counters['done']}/{total_v} | "
                                          f"❌ {batch_counters['failed']}")
                                asyncio.run_coroutine_threadsafe(
                                    update_progress_ui(
                                        current, total_b, _smsg, _start,
                                        f"📥 Downloading [{i}/{total_v}]...", fname,
                                        batch_info=b_info, engine="ytdlp"
                                    ), loop)
                        return _hook
                    _hook = _make_hook(status_msg, start_dl)

                    err = await _process_one_silent(vid_url, per_fmt, status_msg, total_v,
                                                    hook_override=_hook)
                    if err is None:
                        batch_counters["done"] += 1
                    else:
                ...
                # ── Phase 2: Auto-retry once for all failed links ──
                # Collect which urls failed (match by error string)
                retry_urls = []
                for be in list(batch_errors):
                    for bu in batch_urls:
                        if bu[:60] in be or be[:60] in bu:
                            retry_urls.append(bu)
                            break
                # Deduplicate
                retry_urls = list(dict.fromkeys(retry_urls))

                if retry_urls and uid not in abort_dict:
                    await _safe_status_edit(status_msg,
                        f"🔄 <b>Retrying {len(retry_urls)} failed video(s)...</b>")
                    for i, vid_url in enumerate(retry_urls, start=1):
                        if uid in abort_dict:
                            break
                        try:
                            per_fmt = await _resolve_phub_fmt(vid_url)
                            ...
                            per_fmt = "best"
                        err = await _process_one_silent(vid_url, per_fmt, status_msg,
                                                        len(retry_urls))
                        if err is None:
                            batch_counters["done"]   += 1
                            batch_counters["failed"] -= 1   # recovered!
                            # Remove the corresponding error entry
                            for be in list(batch_errors):
                                if vid_url[:60] in be:
                                    with suppress(Exception): batch_errors.remove(be)
                                    break
                        if i < len(retry_urls) and "pornhub" in vid_url:
                            await asyncio.sleep(120)
                        elif i < len(retry_urls):
                            await asyncio.sleep(5)
