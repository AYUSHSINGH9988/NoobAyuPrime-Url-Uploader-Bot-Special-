...
import pyrogram
import yt_dlp
from aiohttp import web  # 🚨 Heroku crash fix ke liye add kiya
...
# ── Dummy Web Server (Heroku Crash Fix) ────────────────────────────────────────

async def handle_ping(request):
    return web.Response(text="Bot is Alive and Running on Heroku!")
...
async def start_webserver():
    ...
    await site.start()
    print(f"[Web] Dummy server started on port {PORT}")
...
async def main():
    ...
    # 🚨 Start dummy web server before starting the bot
...
if __name__ == "__main__":
   # 🚨 Yahan humne existing loop use kiya hai taaki crash na ho
