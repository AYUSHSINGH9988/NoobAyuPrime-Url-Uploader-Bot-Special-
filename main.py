...
# ==========================================
#           BUNKR DECRYPTION ENGINE
# ==========================================
async def get_bunkr_direct_link(url):
    """Scrapes Bunkr URL and decrypts the direct CDN link using API"""
    try:
        ...
        print(f"Bunkr Decryption Error: {e}")
        return None
...
async def download_logic(url, message, user_id, mode, task_info=None, format_id=None):
    try:
        ...
        if mode == "bunkr" or "bunkr" in url.lower():
            ...
            url = bunkr_link # Override URL with the decrypted CDN direct link
            mode = "auto" # Set mode to auto to fall into direct HTTP download
...
async def command_handler(c, m):
    ...
    elif cmd == "bdl": mode = "bunkr"  # Bunkr Decryption trigger
...
async def main():
    ...
    await app.start()
    print("🤖 Bot Started with Bunkr Decryption & Web UI Backend")
