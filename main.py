...
async def upload_file(client, message, file_path, user_mention, task_info=None, batch_info=None,
    ...
    try:
        ...
        # Extract duration natively via yt-dlp if audio/video
        if is_video or is_audio:
            try:
                def _get_dur():
                    import yt_dlp as _ydl
                    with _ydl.YoutubeDL({'quiet': True, 'skip_download': True}) as ydl:
        ...
        # Determine thumbnail
        # Priority 1: Official web thumbnail downloaded alongside the file (for m3u8/streams)
        ...
        if os.path.exists(web_thumb_path) and os.path.getsize(web_thumb_path) > 0:
            ...
            # Priority 3: Auto-generate screenshot from video
