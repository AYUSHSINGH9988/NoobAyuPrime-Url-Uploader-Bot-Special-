    ...
    def _make_progress(fname_display: str, file_idx: int, phase: str):
        ...
        async def _cb(current, total):
            if total == 0 or status.id in abort_dict:
                return
            now = time.time()
            # Allow final 100% update through even if <7s since last
            if now - last_fired[0] < 7.0 and current < total:
                return
            last_fired[0] = now
            # Bypass internal 5s throttle so our 7s window controls rate
    ...
    async def _process_one(src, file_idx: int):
        ...
        try:
            ...
            if src.photo:
                ...
                # ── Fix 1: extract video metadata from original message ───────
                v = src.video
                vid_duration = v.duration or 0
                vid_width    = v.width    or 0
                vid_height   = v.height   or 0
                # Download original thumbnail via user_app (has restricted-chat access)
                if v.thumbs:
                    with suppress(Exception):
                        vid_thumb = await user_app.download_media(
                            v.thumbs[-1].file_id, in_memory=False
                        )
                await c.send_video(
                    target_chat, fp, caption=cap, progress=ul_cb,
                    duration=vid_duration,
                    width=vid_width,
                    height=vid_height,
                    thumb=vid_thumb or None,
            ...
            if vid_thumb:
                with suppress(Exception): os.remove(vid_thumb)
        ...
        if use_all:
            ...
            async for src in user_app.get_chat_history(
                ...
                try:
                    ...
                    # Fix 2: 10s sleep after each upload to avoid API flood bans
