...
async def upload_file(client, message, file_path, user_mention, queue_pos=None):
    try:
        ...
        if active_dump:
            ...
            try:
                await client.send_document(chat_id=target_chat, document=file_path, caption=caption, thumb=thumb_path, progress=update_progress_ui, progress_args=(message, time.time(), upload_status, file_name, queue_pos))
            except Exception as e:
...
async def download_logic(url, message, user_id, mode, queue_pos=None, format_id=None):
    try:
        ...
        # --- 2. PIXELDRAIN (Via /dl) ---
            ...
            # Fallthrough to HTTP logic below...
        ...
        # --- 4. DIRECT HTTP / PIXELDRAIN DOWNLOAD ---
...
async def process_task(client, message, url, mode="auto", upload_target="tg", queue_pos=None, format_id=None):
    ...
    try:
        ...
        if not url and message.reply_to_message:
            ...
            if not media:
                await msg.edit_text("❌ <b>No Media Found!</b>")
                return
            
            # Use getattr to safely get file_name, fallback to ID
            fname = getattr(media, 'file_name', None) or f"tg_file_{message.reply_to_message.id}"
            
            # Handle .torrent file reply for /leech
            if mode == "leech_file":
                if not fname.lower().endswith(".torrent"):
                     await msg.edit_text("❌ <b>Not a .torrent file!</b>\nUse /dl for normal files.")
                     return
                # Download .torrent and pass to aria2 logic
                file_path = await message.reply_to_message.download()
                # Re-route to download_logic with local file path (aria2 handles local .torrent paths)
                # But here we need to read it. Better to just pass file_path to aria2 inside download_logic? 
                # Simplest: Just use Aria2 logic here for .torrent file
                if not aria2: 
                     await msg.edit_text("❌ Aria2 Not Connected!"); return
                ...
                try:
                    ...
                    while True:
                        ...
                        if status.status == "complete": 
                            ...
                            await msg.edit_text("❌ Aria2 Error"); return
                        await update_progress_ui(int(status.completed_length), int(status.total_length), msg, time.time(), "☁️ Torrent Downloading...", status.name, queue_pos)
                        await asyncio.sleep(2)
                except Exception as e:
                    await msg.edit_text(f"❌ Aria2 Error: {e}"); return

            else:
                # Normal TG File Download (/dl)
                file_path = await message.reply_to_message.download(progress=update_progress_ui, progress_args=(msg, time.time(), "📥 Downloading from TG...", fname, queue_pos))
...
async def command_handler(c, m):
    ...
    if cmd == "leech":
        # Check if URL is magnet or torrent
        if url and not ("magnet:" in url or ".torrent" in url.lower()):
            return await m.reply_text("❌ <b>/leech</b> is only for Torrents/Magnets!\nUse <b>/dl</b> for direct links.")
        # Check if reply is .torrent file
        if is_reply:
            doc = m.reply_to_message.document
            if not (doc and doc.file_name and doc.file_name.lower().endswith(".torrent")):
                 return await m.reply_text("❌ <b>/leech</b> is only for .torrent files!\nUse <b>/dl</b> for normal files.")
            mode = "leech_file" # Special mode for torrent file reply
