...
# ✅ FIX 2: Video Timing 0:00 hatane ke liye
...
async def upload_file(client, message, file_path, user_mention, task_info=None, batch_info=None, overall_current=0, overall_total=0, start_time=None):
    try:
        ...
        # ✅ FIX 3: Video duration calculate karna
        thumb_path = None
        duration = 0
            ...
            try:
                if file_name.lower().endswith(('.mp4', '.mkv', '.webm')):
                    await client.send_video(
                        ...
                        duration=duration, # ⬅️ Duration yahan apply ho gayi
                ...
                # ✅ FIX 4: Agar upload fail ho to saaf-saaf error bataye!
        ...
        if mode == "leech" or (url and ("magnet:" in url or ".torrent" in url.lower())):
            ...
            while True:
                ...
                if status.status == "complete":
                    # ✅ FIX 5: Unselected files (jaise JPG/NFO) ko delete karna jo chunk overlap ki wajah se aayi hain
                    for f in status.files:
                        if not f.selected and os.path.exists(str(f.path)):
                            try: os.remove(str(f.path))
                            except: pass
...
async def process_task(client, message, url, mode="auto", upload_target="tg", task_info=None, format_id=None, status_msg=None):
    ...
    try:
        ...
        # ✅ FOLDER EXTRACTION & SELECTOR LOGIC (Ye ab Part 2 wale cleanup ke bharose kaam karega)
        final_files = []
...
async def ytdl_selector(c, m):
    ...
    try:
        ...
        # ✅ YTDL Error Catcher: Heroku par fail ho to saaf saaf error bataye
