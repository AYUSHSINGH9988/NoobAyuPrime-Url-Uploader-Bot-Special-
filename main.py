...
async def scriptdl_handler(c, m):
    ...
    # ─── PHub Profile / Model / Channel / Playlist Batch Detection ───
    batch_urls = []
...
async def scriptdl_quality_cb(c, cb):
    ...
    async def _process_one(vid_url, fmt_id, idx=None, total=None):
        ...
        result = await asyncio.to_thread(_blocking_download, vid_url, fmt_id, out_tmpl, _hook, False)
        if not result:
            shutil.rmtree(dl_dir, ignore_errors=True)
            return False
        fp = result["filepath"]
    ...
    async def _run_scriptdl():
        try:
            if is_batch:
                # ─── Batch loop with 120s anti-ban delay ───
                ok_count = 0
                fail_count = 0
                total_v = len(batch_urls)
                for i, vid_url in enumerate(batch_urls, start=1):
                    if uid in abort_dict:
                        await msg.reply_text("🛑 Batch cancelled.")
                        break
                    # Resolve per-video format id matching the chosen quality
                    per_fmt = "best"
                    try:
                    ...
                    await msg.reply_text(
                        f"▶️ <b>[{i}/{total_v}]</b> Downloading\n<code>{clean_html(vid_url[:80])}</code>"
                    )
                    try:
                        success = await _process_one(vid_url, per_fmt, idx=i, total=total_v)
                        if success: ok_count += 1
                        else: fail_count += 1
                    except Exception as _pe:
                        fail_count += 1
                        await msg.reply_text(f"⚠️ <b>[{i}/{total_v}] Failed:</b> <code>{clean_html(str(_pe))}</code>")

                    # Anti-ban delay (skip after last video)
                    if i < total_v:
                        if "pornhub" in vid_url:
                            await msg.reply_text(
                                "🛡️ <b>Anti-Ban:</b> Waiting 120 seconds before next video extraction..."
                            )
...
def _mega_ls_folder(link: str) -> list:
    """Fetch all file paths inside a MEGA folder link via mega-ls."""
    out, err, code = _run_cmd_ra(["mega-ls", "-r", "--files-only", link], timeout=120)
    if code != 0:
...
async def renameall_handler(c, m):
    """
    Usage: /renameall <mega_folder_link> | <pattern> | <replacement>
    Patterns: prefix | suffix | replace | regex | number | channel
    Examples:
      /renameall https://mega.nz/folder/xxx | prefix | @MyChannel
      /renameall https://mega.nz/folder/xxx | replace | OldText|NewText
      /renameall https://mega.nz/folder/xxx | channel | @mychannel
    """
    if len(m.command) < 2:
        return await m.reply_text(
            "❌ <b>Usage:</b>\n"
            "<code>/renameall &lt;mega_link&gt; | &lt;pattern&gt; | &lt;replacement&gt;</code>\n\n"
            "<b>Patterns:</b>\n"
            "• <code>prefix</code> — Add text before filename\n"
            "• <code>suffix</code> — Add text after filename (before ext)\n"
            "• <code>replace</code> — Replace text: <code>old|new</code>\n"
            "• <code>regex</code>   — Regex replace: <code>pattern|replacement</code>\n"
            "• <code>number</code>  — Sequential numbering (00001.ext ...)\n"
            "• <code>channel</code> — Channel style: <code>@ch (1).ext</code>\n\n"
            "<b>Examples:</b>\n"
            "<code>/renameall https://mega.nz/folder/xxx | prefix | @MyChannel - </code>\n"
            "<code>/renameall https://mega.nz/folder/xxx | replace | OldText|NewText</code>\n"
            "<code>/renameall https://mega.nz/folder/xxx | channel | @mychannel</code>"
        )

    raw_args = m.text.split(None, 1)[1]
    parts    = [p.strip() for p in raw_args.split("|")]
    if len(parts) < 3:
        return await m.reply_text(
            "❌ Format galat hai.\n"
            "Use: <code>/renameall &lt;link&gt; | &lt;pattern&gt; | &lt;replacement&gt;</code>"
        )
    ...
    valid_patterns = {"prefix", "suffix", "replace", "regex", "number", "channel"}
    if pattern not in valid_patterns:
        return await m.reply_text(
            f"❌ Invalid pattern <b>{clean_html(pattern)}</b>\n"
            f"Valid: {', '.join(sorted(valid_patterns))}"
        )

    if not link.startswith("http"):
        return await m.reply_text("❌ Valid MEGA folder link dena bhool gaye!")

    msg = await m.reply_text("🔍 <b>Folder scan ho rahi hai...</b>")

    # ── 1. Fetch file list ──
    try:
        files = await asyncio.wait_for(
            asyncio.to_thread(_mega_ls_folder, link),
            timeout=180
        )
    except asyncio.TimeoutError:
        return await msg.edit_text("❌ Folder scan timeout. Link check karein ya dobara try karein.")
    except Exception as e:
        return await msg.edit_text(f"❌ <b>Folder scan failed:</b>\n<code>{clean_html(str(e))}</code>")

    total = len(files)
    if total == 0:
        return await msg.edit_text("📂 Koi files nahi mili folder mein.")

    # Show preview of first 2 renames
    ex1 = _build_new_name(posixpath.basename(files[0]),  pattern, replacement, 1)
    ex2 = _build_new_name(posixpath.basename(files[min(1, total-1)]), pattern, replacement, 2)

    await msg.edit_text(
        f"📂 <b>Files found: {total:,}</b>\n\n"
        f"👁 <b>Preview:</b>\n"
        f"• <code>{clean_html(posixpath.basename(files[0])[:50])}</code>\n"
        f"  → <code>{clean_html(ex1[:50])}</code>\n"
        f"• <code>{clean_html(posixpath.basename(files[min(1, total-1)])[:50])}</code>\n"
        f"  → <code>{clean_html(ex2[:50])}</code>\n\n"
        f"<b>Pattern:</b> <code>{pattern}</code>\n"
        f"<b>Replacement:</b> <code>{clean_html(replacement)}</code>",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("✅ Start Renaming!", callback_data=f"ra_confirm|{m.id}"),
            InlineKeyboardButton("❌ Cancel",          callback_data=f"ra_cancel|{m.id}"),
        ]])
    )
