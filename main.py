...
# ==========================================
#           CORE LOGIC (Zip, Compress, Extract)
# ==========================================
...
def create_archive(file_path):
    ...
    cmd = ["7z", "a", zip_path, file_path, "-mx1"] # mx1 = Fastest compression
    subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
async def compress_video(file_path, message):
    ...
    # FFmpeg command: Scale 480p, CRF 28 (Low Qual), Ultrafast (Low CPU)
    cmd = [
        "ffmpeg", "-i", file_path,
        "-vf", "scale=-2:480", 
...
async def process_task(client, message, url, mode="auto", upload_target="tg", queue_pos=None, format_id=None):
    ...
    try:
        ...
        # 1. COMPRESS (If mode is compress and file is video)
        ...
        # 3. EXTRACT (If normal mode and file is archive)
...
async def command_handler(c, m):
    ...
    if cmd == "queue":
        ...
        await m.reply_text(f"✅ <b>Added {len(text.split())} Tasks!</b>")
