...
async def download_logic(url, message, mode):
    try:
        ...
        # 1. ARIA2 (Magnet/Torrent) - FIXED POSIX PATH ERROR
        if url.startswith("magnet:") or url.endswith(".torrent"):
            ...
            try:
                ...
                while True:
                    ...
                    # Force stop seeding if download is complete
                ...
                # FIX: Convert Path Object to String immediately
                if download.files and len(download.files) > 0:
                    return str(download.files[0].path), None
                else:
                    return None, "❌ File Path Error"

            except Exception as e: return None, f"❌ Aria2 Error: {e}"

        # 2. YT-DLP MODE (Strictly ONLY if mode is ytdl)
        if mode == "ytdl":
            try:
        ...
        # 3. DIRECT HTTP (Default for /leech)
        if mode == "direct":
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as resp:
...
async def process_queue():
    ...
    while task_queue:
        ...
        try:
            ...
            # FIX: Ensure file_path is string for .lower()
            str_path = str(file_path)
            
            is_archive = str_path.lower().endswith(('.zip', '.rar', '.7z', '.tar', '.gz', '.iso'))
            is_split = bool(re.search(r'\.\d{3}$', str_path))
            ...
            if is_archive or is_split:
                ...
                cmd = ["7z", "x", str_path, f"-o{output_dir}", "-y"]
            ...
            if DUMP_CHANNEL:
                try:
                    pin_name = os.path.splitext(os.path.basename(final_files[0]))[0]
