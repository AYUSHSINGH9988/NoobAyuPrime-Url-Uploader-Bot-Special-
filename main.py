...
async def _start_ytdl(uid, fmt, session, msg, client):
    ...
    async def _run():
        try:
            ...
            await msg.edit_text(
                f"✅ <b>Done!</b> Sent to {dest}\n"
                f"<b>Engine:</b> <code>yt-dlp {YTDLP_VERSION}</code> | <code>PyroTgfork {PYROGRAM_VERSION}</code>"
            )
        except Exception as ex:
            traceback.print_exc()
            await msg.edit_text(f"⚠️ <b>Error:</b> <code>{clean_html(str(ex))}</code>")
...
async def scriptdl_quality_cb(c, cb):
    ...
    if fmt_index >= len(formats):
        await cb.message.edit_text("❌ Invalid format selected.")
        return

    selected_fmt = formats[fmt_index]
    direct_url   = selected_fmt.get("url")
    ext          = selected_fmt.get("ext", "mp4")
    uid          = session["uid"]
    raw_title    = session["title"]
    title_safe   = re.sub(r'[<>:"/\\|?*]', "_", raw_title).strip()[:100] or "video"
    ...
    if not direct_url:
        await msg.edit_text("❌ No direct URL found for this format. Try another quality.")
        return

    await msg.edit_text(
        f"⏳ <b>Starting ScriptDL Download...</b>\n"
        f"<code>{clean_html(direct_url[:80])}</code>"
    )
    ...
    async def _run_scriptdl():
        try:
            ...
            await upload_file(
                ...
                task_info="ScriptDL", batch_info=filename,
