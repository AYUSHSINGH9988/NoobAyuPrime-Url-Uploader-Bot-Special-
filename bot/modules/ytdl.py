...
Flags:
  -vt   → show Video Tools menu after download instead of uploading
  -n    → rename downloaded file
  -b    → bulk mode (multiple URLs, one per line)
"""
...
async def ytdl_cmd(c, m):
    ...
    if len(args) < 2:
        return await m.reply_text(
            "❌ Usage: /ytdl URL [-n rename] [-b bulk] [-vt video tools]"
        )
    ...
    if bulk:
        ...
        for i, url in enumerate(urls, 1):
            await _ytdl_download_and_upload(
                c, m, url, "bv*+ba/b", uid, rename,
                task_info=f"URL {i}/{len(urls)}", do_vt=do_vt,
            )
        return
...
async def ytq_cb(c, cb):
    ...
    await _ytdl_download_and_upload(
        ...
        is_audio=is_audio, do_vt=do_vt,
    )


async def _ytdl_download_and_upload(
    c, msg, url, fmt, uid, rename=None,
    task_info=None, is_audio=False, do_vt=False,
):
    proxy    = (await get_user_proxy(uid)) or PROXY_URL
    ...
    def _hook(d):
        ...
        if current > 0:
            asyncio.run_coroutine_threadsafe(
                update_progress_ui(
                    current, total, msg, start, "📥 Downloading...",
    ...
    try:
        ...
        if do_vt:
            ...
            user_name = (await c.get_me()).first_name  # fallback
            try:
                # Try to get the actual user's name from the message context
...
async def _do_playlist(c, msg, url, fmt, uid, quality, proxy):
    ...
    def _hook(d):
        if d["status"] == "downloading":
            ...
            asyncio.run_coroutine_threadsafe(
                update_progress_ui(
                    current, total, msg, start, f"📥 Video {idx['n']}...",
                    fname, engine="ytdlp",
                ),
                loop,
            )
