...
/leech, /dl, /queue, /stopseed
  - /leech link          → torrent/magnet download
  - /leech link -s       → show file selection UI
  - /dl link             → direct download (HTTP / yt-dlp / GDrive)
  - /dl reply -z         → zip replied file
  - /dl reply -e         → extract replied archive
  - /dl link -vt         → download then show Video Tools menu
  - /leech link -vt      → download torrent then show Video Tools menu
"""
...
async def _queue_manager(client, user_id):
    ...
    try:
        while user_queues.get(user_id):
            url, msg, mode, target, rename, seed, do_vt = user_queues[user_id].pop(0)
            await process_task(client, msg, url, mode, target, rename=rename, seed=seed, do_vt=do_vt)
    finally:
...
async def leech_cmd(c, m):
    ...
    if is_reply:
        ...
        asyncio.create_task(
            process_task(c, m, fp, "leech_file", seed=seed, user_id=m.from_user.id, do_vt=do_vt)
        )
        return
    ...
    if not url:
        return await m.reply_text(
            ...
            "• <code>/leech ... -vt</code> — open Video Tools after download\n"
    ...
    asyncio.create_task(
        process_task(c, m, url, "leech", seed=seed, rename=rename, user_id=m.from_user.id, do_vt=do_vt)
    )
...
async def dl_cmd(c, m):
    ...
    do_vt  = "-vt" in raw.split()

    for f in (" -b", " -z", " -zip", " -e", " -extract", " -vt"):
        raw = raw.replace(f, "")
    if " -n " in raw:
        parts  = raw.split(" -n ", 1)
        raw    = parts[0]
    ...
    if len(args) < 2:
        return await m.reply_text(
            "❌ Send a URL or reply to a file.\n\n"
            "Flags:\n"
            "• <code>-z</code>    — zip file after download\n"
            "• <code>-e</code>    — extract archive after download\n"
            "• <code>-vt</code>   — open Video Tools menu after download\n"
            "• <code>-n name</code> — rename file"
        )
    ...
    if bulk and len(links) > 1:
        ...
        for l in links:
            user_queues[uid].append((l, m, mode, "tg", rename, False, do_vt))
    ...
    for l in links:
        asyncio.create_task(
            process_task(c, m, l, mode, rename=rename, user_id=uid, do_vt=do_vt)
        )
...
async def queue_cmd(c, m):
    ...
    user_queues.setdefault(uid, [])
    for u in urls:
        user_queues[uid].append((u, m, "auto", "tg", None, False, False))
