...
async def _send_file(client, dest_chat, file_path, caption, thumb_path,
                     send_as, is_video, is_audio, duration, progress_cb):
    """Attempt to send a file to dest_chat. Returns sent message or raises."""
    if send_as == "document":
...
async def upload_file(
    ...
    try:
        ...
        progress_cb = _progress if file_size > 10 * 1024 * 1024 else None

        sent_msg = None
        try:
            sent_msg = await _send_file(
                client, dest_chat, file_path, caption, thumb_path,
                send_as, is_video, is_audio, duration, progress_cb,
            )
        except Exception as e:
            err_str = str(e)
            # PeerIdInvalid: dump channel no longer accessible — fall back to user PM
            if "PeerIdInvalid" in err_str and dest_chat != message.chat.id:
                try:
                    with suppress(Exception):
                        await message.reply_text(
                            "⚠️ <b>Dump channel unreachable (PeerIdInvalid).</b>\n"
                            "Sending to your chat instead..."
                        )
                    sent_msg = await _send_file(
                        client, message.chat.id, file_path, caption, thumb_path,
                        send_as, is_video, is_audio, duration, progress_cb,
                    )
                except Exception as e2:
                    with suppress(Exception):
                        await message.reply_text(
                            f"❌ <b>Upload Error (fallback also failed):</b>\n"
                            f"<code>{clean_html(str(e2))}</code>"
                        )
                    return False
            else:
                with suppress(Exception):
                    await message.reply_text(
                        f"❌ <b>Upload Error:</b> <code>{clean_html(err_str)}</code>"
                    )
                return False
